...
#define dimT 100000 // Number of t bins, may need to increase
#define DSIZE (100*100*1000) // 10 million
#define SAVESIZE 100  // Should be dimT/10
...
__global__ void finiteDiff(const float *inputVal, float *outputVal, float *saveCenter, int ds, int tStamp){
    ...
    for (int index = threadIdx.x+blockDim.x*blockIdx.x; index < ds; index+=gridDim.x*blockDim.x){   
        ...
        if(z==500){
          saveCenter[tStamp*dimX*dimY + x + y*dimX] = outputVal[index];
        }
    }
}
...
void stepAlgo(float *d_inputVal, float *d_outputVal, float *d_saveCenter, int tStamp){

    // Run the main algorithm
    finiteDiff<<<blocks, threads>>>(d_inputVal, d_outputVal, d_saveCenter, DSIZE, tStamp);
    cudaCheckErrors("main kernel launch failure");
    cudaDeviceSynchronize();

}

void run(float *inputVal, float *saveCenter){
    int counter = 1;
    ...
    //cudaMalloc(&d_saveData, DSIZE*SAVESIZE*sizeof(float));
    cudaMalloc(&d_saveCenter, dimX*dimY*dimT*sizeof(float));
    ...
    while(counter<=dimT) {
        ...
        if((counter-1)%100==0){
            ...
            stepAlgo(d_inputVal, d_saveData+DSIZE*(counter-1)/100, counter-1);
            counter++;
    
            // Run another step but with the input and output arrays flipped so
            // the memory doesn't need copied        
            stepAlgo(d_saveData+DSIZE*(counter-2)/100, d_inputVal, counter-1);
            counter++;
            */
        }
      // Run 1 step of the algorithm
            stepAlgo(d_inputVal, d_outputVal, d_saveCenter, counter-1);
            counter++;
            
            // Run another step but with the input and output arrays flipped so
            // the memory doesn't need copied
            stepAlgo(d_outputVal, d_inputVal, d_saveCenter, counter-1);
            counter++;
    ...
    cudaMemcpy(saveCenter, d_saveCenter, dimX*dimY*dimT*sizeof(float), cudaMemcpyDeviceToHost);
...
int main(int argc, char *argv[]){
    ...
    float *saveCenter = new float[dimX*dimY*dimT];
    ...
    fwrite(saveCenter, sizeof(float), dimX*dimY*dimT, data);
    fclose(data);
