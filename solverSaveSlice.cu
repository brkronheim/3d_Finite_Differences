...
#define SSIZE (dimX*dimY*dimT) // Size of slice to save, total size is 400 MB
...
__global__ void finiteDiff(const float *inputVal, float *outputVal, float *saveSlice, int ds, int tStamp){
    ...
    for (int index = threadIdx.x+blockDim.x*blockIdx.x; index < ds; index+=gridDim.x*blockDim.x){   
        ...
        if(z==(int)dimZ/2){
          saveSlice[tStamp*dimX*dimY + x + y*dimX] = outputVal[index];
        }
    }
}
...
void stepAlgo(float *d_inputVal, float *d_outputVal, float *d_saveSlice, int tStamp){

    // Run the main algorithm
    finiteDiff<<<blocks, threads>>>(d_inputVal, d_outputVal, d_saveSlice, DSIZE, tStamp);
    cudaCheckErrors("main kernel launch failure");
    cudaDeviceSynchronize();

}

void run(float *inputVal, float *saveSlice){
    int counter = 1;
    ...
    while(counter<=dimT) {
        ...
        stepAlgo(d_inputVal, d_outputVal, d_saveSlice, counter-1);
        counter++;
    
        // Run another step but with the input and output arrays flipped so
        // the memory doesn't need copied
        stepAlgo(d_outputVal, d_inputVal, d_saveSlice, counter-1);
        counter++;
    ...
    cudaMemcpy(saveSlice, d_saveSlice, dimX*dimY*dimT*sizeof(float), cudaMemcpyDeviceToHost);
...
int main(int argc, char *argv[]){
    ...
    fwrite(saveSlice, sizeof(float), dimX*dimY*dimT, data);
    fclose(data);
